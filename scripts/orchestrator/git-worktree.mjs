...
export function createGitWorktreeManager({ projectRoot: projectRootInput, runGit = null } = {}) {
  ...
  const worktreesRoot = taskWorkspaces.workspaceRoot;
  const restoresRoot = join(worktreesRoot, 'restores');
  function workspaceRootFor(task) {
    if (!task.workspaceRootAbs) return taskWorkspaces.reserve(task).workspaceRootAbs;
    const expected = resolve(task.workspaceRootAbs);
    if (!inside(worktreesRoot, expected)) fail('TASK_WORKSPACE_ESCAPE', 'task workspace path escapes work');
    const canonical = realpathSync.native(expected);
    if (!inside(worktreesRoot, canonical)) fail('TASK_WORKSPACE_ESCAPE', 'canonical task workspace path escapes work');
    return canonical;
  }
  ...
  function pathFor(task) { return join(workspaceRootFor(task), 'repo'); }
  function prepare(task) {
    ...
    const workspaceRootAbs = workspaceRootFor(task);
    const expected = join(workspaceRootAbs, 'repo'); if (!inside(worktreesRoot, expected)) fail('TASK_WORKTREE_ESCAPE', 'worktree path escapes work');
    ...
    const canonical = realpathSync.native(expected); if (!inside(worktreesRoot, canonical)) fail('TASK_WORKTREE_ESCAPE', 'canonical worktree path escapes work');
    ...
    return { workspaceRootAbs, worktreePathAbs: canonical, inputCommit: head };
  }
  ...
  function restoreSnapshot({ targetProjectRootAbs, snapshotId, outputCommit }) {
    ...
    const expected = taskWorkspaces.restorePathFor(snapshotId);
