...
import { MAX_AGENT_TIMEOUT_SECONDS, validateAgentTimeoutSeconds } from './agent-process.mjs';
import { terminateProcessTree } from './process-utils.mjs';
import { cleanupTestSandboxSession, prepareTestSandboxSession, assertSandboxAttestation } from './sandbox-runtime.mjs';
...
function mappedSandboxPath(path, mountPlan) {
  if (!mountPlan) return path;
  const hostPath = resolve(path);
  for (const mount of mountPlan.mounts) {
    const mountHost = resolve(mount.host_path_abs);
    const relativePath = relative(mountHost, hostPath);
    if (relativePath === '' || (relativePath !== '..' && !relativePath.startsWith(`..${sep}`) && !isAbsolute(relativePath))) {
      return join(mount.container_path, relativePath).replaceAll('\\', '/');
    }
  }
  return path;
}

function taskMessage(task, intent, sandboxMountPlan = null, sandboxAttestation = null) {
  const outputInstructions = task.structured_outputs.map((output) => ({
    final_output_path_abs: sandboxMountPlan ? '<host-only-final-output-written-by-orchestrator>' : output.path_abs,
    staged_raw_path_abs: mappedSandboxPath(join(task.artifact_root_abs, '.agent-raw', `${output.path_abs.slice(task.artifact_root_abs.length).replace(/^[\\/]/u, '')}.raw`), sandboxMountPlan),
  ...
  return `# Local Orchestrator Task\n\nThis message is generated by the local workflow Orchestrator. It is not a request to decide workflow state.\n\n`
    ...
    + `- context_manifest_path_abs: ${sandboxMountPlan ? '/input/context-manifest.json' : task.context_manifest_path_abs}\n- worktree_path_abs: ${sandboxMountPlan ? '/worktree' : task.worktree_path_abs}\n- artifact_root_abs: ${sandboxMountPlan ? '<host-only-artifact-root>' : task.artifact_root_abs}\n\n`
    + (sandboxMountPlan
      ? 'Sandbox boundary: this test-agent run is mandatory SANDBOXED_DOCKER. The host filesystem is not available inside the container. Use only /worktree (rw), /input (ro), /agent-raw (rw), /raw-logs (rw), with cwd /workspace. Do not use host paths for file access, request elevated execution, or fall back to host execution. The staged input/task.json and input/context-manifest.json retain host paths as metadata required by the output contract.\n\n'
      : '')
    + (sandboxAttestation
      ? `The Orchestrator has verified this runtime. Copy this exact object into the staged result JSON as both isolation_mode=SANDBOXED_DOCKER and sandbox_attestation; do not invent or modify its fields:\n\n\`\`\`json\n${JSON.stringify({ isolation_mode: 'SANDBOXED_DOCKER', sandbox_attestation: sandboxAttestation }, null, 2)}\n\`\`\`\n\n`
      : '')
...
function runtimeRootFromTask(task) {
  const worktreesRoot = resolve(task.worktree_path_abs, '..', '..', '..', '..', '..');
  const artifactsRoot = resolve(task.artifact_root_abs, '..', '..', '..', '..');
  if (worktreesRoot !== artifactsRoot) throw new OrchestratorError('SANDBOX_RUNTIME_ROOT_MISMATCH', 'test worktree and artifact paths do not share the same runtime root');
...
export function startOpenClawAgent({ agentId, sessionId, messagePath, timeoutSeconds = MAX_AGENT_TIMEOUT_SECONDS, launcher, sandboxLeasePath = null } = {}) {
...
export async function startReadyTask({ projectRoot: projectRootInput, databasePath, taskId, timeoutSeconds = null,
  ...
  try {
    ...
    try {
      ...
      sandbox = task.assigned_agent === 'test-agent'
        ? await prepareTestSandboxSession({ projectRoot, task, sessionId, sessionKey: intent.session_key,
          runtimeRootAbs: runtimeRootFromTask(task), commandRunner: sandboxCommandRunner })
        : null;
      if (sandbox) atomicWriteFile(messagePath, taskMessage(task, intent, sandbox.mountPlan, sandbox.attestation));
      if (sandbox) atomicWriteJson(launcher.launcher_path_abs, {
        ...JSON.parse(readFileSync(launcher.launcher_path_abs, 'utf8')),
        sandbox_lease_path_abs: sandbox.leasePath,
        sandbox_mount_plan: sandbox.mountPlan,
      });
      const launched = await runner({ agentId: task.assigned_agent, sessionId, messagePath,
        timeoutSeconds: execution.timeoutSeconds, launcher, sandboxLeasePath: sandbox?.leasePath ?? null });
      atomicWriteJson(launcher.launcher_path_abs, {
        ...JSON.parse(readFileSync(launcher.launcher_path_abs, 'utf8')),
      ...
      return {
        ...
        dispatch_id: intent.dispatch_id,
        session_id: sessionId,
        launcher: { ...launcher, launcher_pid: launched.launcher_pid ?? null, sandbox_lease_path_abs: sandbox?.leasePath ?? null },
      };
    } catch (error) {
      if (sandbox?.lease) {
        try { await cleanupTestSandboxSession({ lease: sandbox.lease, leasePath: sandbox.leasePath, commandRunner: sandboxCommandRunner }); }
        catch (cleanupError) { error = new OrchestratorError('SANDBOX_CLEANUP_FAILED', `${error.message}; cleanup: ${cleanupError.message}`); }
      }
...
export function reconcileDispatch({ projectRoot: projectRootInput, databasePath, dispatchId, clock = () => new Date(), uuid = randomUUID } = {}) {
  ...
  try {
    ...
    if (task.assigned_agent === 'test-agent') {
      const launcherRecord = JSON.parse(readFileSync(launcher.launcher_path_abs, 'utf8'));
      if (!launcherRecord.sandbox_lease_path_abs || !existsSync(launcherRecord.sandbox_lease_path_abs)) {
        throw new OrchestratorError('SANDBOX_LEASE_MISSING', 'test-agent dispatch has no sandbox lease evidence');
      }
      const processAttestation = processResult.sandbox_attestation;
      if (!processAttestation) throw new OrchestratorError('SANDBOX_ATTESTATION_MISSING', 'test-agent process completed without sandbox attestation');
      const lease = JSON.parse(readFileSync(launcherRecord.sandbox_lease_path_abs, 'utf8'));
      assertSandboxAttestation(processAttestation, lease.mount_plan);
    }
    ...
    if (task.assigned_agent === 'test-agent') {
      if (rawResult.isolation_mode !== 'SANDBOXED_DOCKER' || !rawResult.sandbox_attestation) {
        throw new OrchestratorError('SANDBOX_RESULT_ATTESTATION_MISSING', 'test-agent result must declare SANDBOXED_DOCKER and sandbox_attestation');
      }
    }
    const completion = {
