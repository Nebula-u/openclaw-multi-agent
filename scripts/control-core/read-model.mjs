...
export function createControlSnapshot(database, { workflowId = null } = {}) {
  ...
  const workflows = workflowRows.map((row) => {
    const tasks = tableExists(database, 'tasks')
      ? database.prepare('SELECT * FROM tasks WHERE workflow_id=? ORDER BY created_at, task_id').all(row.workflow_id).map((taskRow) => {
        const dispatches = database.prepare('SELECT * FROM dispatches WHERE task_id=? ORDER BY created_at').all(taskRow.task_id).map((dispatchRow) => ({
      ...
      : [];
    return { ...parseJson(row.state_json), tasks };
  });
  let supervision = [];
