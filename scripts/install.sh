#!/usr/bin/env bash
# =============================================================================
# openclaw-sdlc-multi-agent 安装脚本（Bash 等价实现，Windows 主实现见 install.ps1）
#
...
# ---------------------------------------------------------------------------
# 1. 7 个 Agent 规范定义 + 绝对 workspace/agentDir
# ---------------------------------------------------------------------------
...
# 预构造 worker allowAgents 的 JSON 数组文本（复用，避免内联引号出错）
WORKER_ALLOW_JSON="$(printf '"%s",' "${WORKER_IDS[@]}")"
WORKER_ALLOW_JSON="[${WORKER_ALLOW_JSON%,}]"

declare -A WS DIR MODEL
for id in "${AGENT_IDS[@]}"; do
  WS[$id]="$(normpath "$RUNTIME_ROOT_ABS/agents/$id/workspace")"
  DIR[$id]="$(normpath "$RUNTIME_ROOT_ABS/agents/$id/state")"
  MODEL[$id]=""
  assert_abs "${WS[$id]}"  "$id.workspace"
  assert_abs "${DIR[$id]}" "$id.agentDir"
done

# 可选模型配置（需要 jq）
if [ -n "$MODEL_CONFIG" ]; then
  case "$MODEL_CONFIG" in /*) : ;; *) MODEL_CONFIG="$(normpath "$MODEL_CONFIG")";; esac
  if [ -f "$MODEL_CONFIG" ] && command -v jq >/dev/null 2>&1; then
    for id in "${AGENT_IDS[@]}"; do
      m="$(jq -r --arg id "$id" '.agents[$id].model // empty' "$MODEL_CONFIG" 2>/dev/null || true)"
      [ -n "$m" ] && MODEL[$id]="$m"
    done
  else
    echo "警告: 无法读取 ModelConfig（文件不存在或缺少 jq），忽略：$MODEL_CONFIG" >&2
  fi
fi
...
EXISTING_IDS=""
if [ "$HAS_JQ" -eq 1 ]; then
  EXISTING_IDS="$(openclaw agents list --json 2>/dev/null | jq -r '.[].id' 2>/dev/null | tr '\n' ' ' || true)"
fi
...
for id in "${AGENT_IDS[@]}"; do
  RUNTIME_DIRS+=("${WS[$id]}" "${DIR[$id]}" "${WS[$id]}/rules")
done

# ---------------------------------------------------------------------------
# 4. 冲突检测：同名 Agent（仅在有 jq 时精确比对 workspace）
# ---------------------------------------------------------------------------
if [ "$HAS_JQ" -eq 1 ] && [ -n "$EXISTING_IDS" ]; then
  CONFLICT=0
  for id in "${AGENT_IDS[@]}"; do
    if contains "$EXISTING_IDS" "$id"; then
      ex_ws="$(openclaw agents list --json 2>/dev/null | jq -r --arg id "$id" '.[] | select(.id==$id) | .workspace // ""')"
      if [ -n "$ex_ws" ] && [ "$ex_ws" != "${WS[$id]}" ]; then
        echo "  - Agent '$id' 已存在且 workspace 不同：现有=$ex_ws 期望=${WS[$id]}" >&2
        CONFLICT=1
      else
...
for id in "${AGENT_IDS[@]}"; do
  mdl="${MODEL[$id]:-(继承默认)}"
  printf "  %-18s ws=%s\n" "$id" "${WS[$id]}"
  printf "  %-18s dir=%s  model=%s\n" "" "${DIR[$id]}" "$mdl"
done
echo "  manager subagents.allowAgents = ${WORKER_IDS[*]}"
echo "  manager subagents.requireAgentId = true ; delegationMode = prefer"
echo "  worker  subagents.allowAgents = [] (禁止再派生)"
echo "  test-agent sandbox.mode = off (本阶段无沙箱的显式声明)"
...
write_manifest() {
  ...
  for id in "${AGENT_IDS[@]}"; do
    local allow="[]" reqid="false" sb="null"
    if [ "$id" = "manager-agent" ]; then
      allow="$WORKER_ALLOW_JSON"
      reqid="true"
    fi
    [ "$id" = "test-agent" ] && sb="\"off\""
    [ $first -eq 0 ] && agents_json="$agents_json,"
    first=0
    local ews edir emdl
    ews="$(json_escape "${WS[$id]}")"; edir="$(json_escape "${DIR[$id]}")"; emdl="$(json_escape "${MODEL[$id]}")"
    agents_json="$agents_json{\"id\":\"$id\",\"workspace_abs\":\"$ews\",\"agentDir_abs\":\"$edir\",\"model\":\"$emdl\",\"subagents_allow\":$allow,\"require_agent_id\":$reqid,\"sandbox_mode\":$sb}"
  done
  local ecfg epr err
...
if [ "$APPLY" -eq 0 ]; then
  ...
  for id in "${AGENT_IDS[@]}"; do
    ...
    echo "  openclaw agents add $id --non-interactive --workspace \"${WS[$id]}\" --agent-dir \"${DIR[$id]}\"$modelarg --json"
  done
  echo ""
...
# ---------------------------------------------------------------------------
if [ "$HAS_JQ" -ne 1 ]; then
  echo "APPLY 需要 jq 来安全定位 agents.list 索引。请安装 jq 后重试，或改用 install.ps1。" >&2
  echo "（本脚本不会自动安装任何依赖。）" >&2
  exit 1
fi

if [ "$ASSUME_YES" -ne 1 ]; then
  echo ""
  echo "即将修改 OpenClaw 配置并创建 7 个 Agent。"
...
restore_on_failure() {
  ...
  if [ "$BACKUP_JSON" != "null" ]; then
    ...
    if [ -f "$bp" ] && [ -n "$CONFIG_FILE" ]; then
      cp -f "$bp" "$CONFIG_FILE" && echo "[恢复] 已从备份还原配置：$bp" >&2
    fi
  else
...
SRC_COMMON="$SRC_AGENTS/common"
SRC_TEMPLATES="$PROJECT_ROOT/templates"
for id in "${AGENT_IDS[@]}"; do
  src_ws="$SRC_AGENTS/$id/workspace"
  dst_ws="${WS[$id]}"
  if [ -d "$src_ws" ]; then
    cp -Rf "$src_ws/." "$dst_ws/"
  fi
...
if [ -d "$SRC_TEMPLATES" ]; then
  mgr_tpl="${WS[manager-agent]}/templates"
  mkdir -p "$mgr_tpl"
  cp -Rf "$SRC_TEMPLATES/." "$mgr_tpl/"
fi
...
for id in "${AGENT_IDS[@]}"; do
  ...
  add_args=(agents add "$id" --non-interactive --workspace "${WS[$id]}" --agent-dir "${DIR[$id]}" --json)
...
agent_index() {
  openclaw config get "agents.list" --json 2>/dev/null \
    | jq -r --arg id "$1" 'to_entries[] | select(.value.id==$id) | .key' | head -1
}
...
for id in "${AGENT_IDS[@]}"; do
  ...
  if [ "$id" = "manager-agent" ]; then
    ...
    set_json "agents.list[$idx].subagents" "{\"allowAgents\":[]}"
    if [ "$id" = "test-agent" ]; then
      set_json "agents.list[$idx].sandbox" "{\"mode\":\"off\"}"
    fi
  fi
done
...
if [ "$SET_MANAGER_DEFAULT" -eq 1 ]; then
  mi="$(agent_index manager-agent)"
  if openclaw config set "agents.list[$mi].default" "true" --strict-json >/dev/null 2>&1; then
    CONFIG_CHANGES+=("set manager-agent.default=true")
  else
    echo "警告: 设置 manager 默认失败" >&2
  fi
fi
if [ -n "$MANAGER_BINDING" ]; then
  if openclaw agents bind manager-agent --bind "$MANAGER_BINDING" >/dev/null 2>&1; then
    CONFIG_CHANGES+=("bind manager-agent $MANAGER_BINDING")
  else
