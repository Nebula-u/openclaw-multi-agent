...
function requiredConfig(config, field) {
  const value = config?.configurable?.[field];
  if (typeof value !== 'string' || value.length === 0) throw new Error(`LangGraph checkpoint config requires ${field}`);
  return value;
}
...
export class SqliteCheckpointSaver extends BaseCheckpointSaver {
  ...
  async deserializeRow(row) {
    ...
    if (row.parent_checkpoint_id) value.parentConfig = checkpointConfig(row.thread_id, row.checkpoint_ns, row.parent_checkpoint_id);
    return value;
  }
  ...
  threadIds() {
    return this.database.prepare(`SELECT thread_id, MAX(created_at) AS updated_at
      FROM langgraph_checkpoints GROUP BY thread_id ORDER BY updated_at DESC`).all();
  }
