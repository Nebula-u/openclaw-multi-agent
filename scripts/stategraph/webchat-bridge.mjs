...
const APPROVE = /^(?:\/workflow\s+approve|确认|同意|批准|可以|就这么走|按这个(?:方案|路线)(?:走|执行)|开始执行)[。！!\s]*$/u;
...
export function createWebchatWorkflowBridge({ runtime, projectPath, maxTurns = 8 } = {}) {
  ...
  async function handle({ text, sessionKey, senderId }) {
    ...
    if (current?.condition === 'WAITING_HUMAN') {
      if (!APPROVE.test(text.trim())) return { handled: true, reply: formatWorkflowReply(current) };
      if (!current.pendingApproval?.options?.some((option) => option.id === 'APPROVE')) return { handled: true, reply: formatWorkflowReply(current, '当前审批不支持直接确认。\n') };
      await runtime.approve(current.workflowId, { decision_id: current.pendingApproval.decision_id, choice: 'APPROVE', decided_by: actor(senderId), notes: text.trim(), decided_at: new Date().toISOString() });
