(function () {
  ...
  async function openTask(task) { const dialog = $('task-dialog'); const dispatch = latestDispatch(task); let activities = []; try { activities = (await request(`/api/tasks/${encodeURIComponent(task.task_id)}/activity`)).activities; } catch (_) { activities = []; }
    $('task-detail').innerHTML = `<span class="eyebrow">${escapeHtml(task.task_id)}</span><h2 class="detail-title">${escapeHtml(task.title || task.task_type)}</h2><div class="detail-grid">
      ...
      <div class="nudge-form"><label for="request-type">监督动作</label><select id="request-type"><option value="NUDGE">催办进度</option><option value="SEND_MESSAGE">发送补充说明</option><option value="RECONCILE">核查 session</option><option value="RETRY_REVIEW">申请 retry review</option><option value="PAUSE_REQUEST">请求暂停</option><option value="RESUME_REQUEST">请求恢复</option><option value="CANCEL_REQUEST">请求取消</option><option value="ESCALATE">升级人工处理</option></select><label for="nudge-reason">请求说明</label><textarea id="nudge-reason">请汇报当前 checkpoint、已完成事实、阻塞原因和下一步；不要重新执行已完成副作用。</textarea><button id="nudge-button" class="button primary">创建监督请求</button></div>`;
    $('nudge-button').addEventListener('click', () => createSupervisionRequest(task)); dialog.showModal(); }
  async function createSupervisionRequest(task) { const dispatch = latestDispatch(task); const now = new Date(); const uuid = crypto.randomUUID(); const reason = $('nudge-reason').value.trim(); const requestType = $('request-type').value;
    if (!reason) return alert('请填写请求说明。');
    if (requestType !== 'NUDGE' && !confirm(`${requestType} 将交由 manager 审计和核查后执行，不会由看板直接改变任务状态。继续创建请求？`)) return;
    const body = { schema_version:1, request_id:`SUP-${uuid}`, idempotency_key:`${task.workflow_id}/${task.task_id}/${task.run_id}/${requestType}/${Math.floor(now.valueOf()/300000)}`,
      ...
      request_type:requestType, source:'LOCAL_USER', reason, evidence:{ source:'static-dashboard', requested_for_status:task.status, health:task.health || null }, requested_at:now.toISOString() };
