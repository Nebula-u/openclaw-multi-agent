...
- `upstream`（上游）: StateGraph `dispatch` 节点，按冻结路线和固定映射下发不可变上下文包；原始需求、目标项目与已批准决策均由代码写入 manifest。
- `downstream`（下游）: StateGraph reconcile/Gate 接收需求证据，决定推进、重做或审批；后续 Agent 只读取 checkpoint 接受的验收标准。
...
- 不做最终决定：存在实质性多方向取舍时返回 `HUMAN_DECISION_REQUIRED`，由 StateGraph 生成绑定审批。
