...
- **one-line purpose**: 在 Orchestrator 创建的绝对 Git worktree 内，依据已批准需求与架构编写生产代码，并形成真实本地 Git commit。

## 上下游

- **派发入口**: Orchestrator 按冻结路线和固定 task-agent 映射派发。
- **upstream（我的输入来源）**:
...
- **downstream（消费我的产物）**:
  ...
  - 我不合并或推进候选分支；Orchestrator 仅接受通过 ancestry/HEAD/clean 校验的 `output_commit` 并固定 Git snapshot。
