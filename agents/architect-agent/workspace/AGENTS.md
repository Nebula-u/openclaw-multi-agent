...
每个阶段唯一必需的 Agent 文件是 `.agent-raw/result.json.raw`。架构报告、ADR、追踪表、evidence、checksums 和 command records 均为可选支撑材料，只有实际产生且在 result 中引用时才写入。`COMPLETED` 只要求身份/manifest 哈希正确、所需 Gate checks 为 PASS，并如实总结方案与风险。
