...
集成分工提醒：CLI 调用发生在**安装/配置阶段**（`install.ps1` / `install.sh`）；日常工作流依赖 OpenClaw 原生 Agent、原生工具、Node Orchestrator、单机 SQLite 与本地 Git。用户访问继续通过 OpenClaw Gateway 和 manager-agent 的 channel binding；项目不再安装或运行 StateGraph WebChat 插件。
